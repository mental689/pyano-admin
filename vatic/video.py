...
def test_extract(video_id='fs-ifFsc8Bg', output_dir='./static/frames'):
    extractor = Extractor(video_path='./static/videos/{}.mp4'.format(video_id),
...
class Loader(object):
    ...
    def __call__(self, group):
        ...
        path = Video.getframepath(0, self.location)
        try:
            im = Image.open(path)
        except IOError:
            logger.debug("Cannot read {0}".format(path))
            return
        ...
        path = Video.getframepath(maxframes - 1, self.location)
        try:
            im = Image.open(path)
        except IOError:
            logger.debug("Cannot read {0}".format(path))
        ...
        if Video.objects.filter(slug=self.slug).count() > 0:
            logger.debug("Video {0} already exists!".format(self.slug))
            return

        if self.trainwith_id:
            if self.for_training:
                logger.debug("A training video cannot require training")
                return
        ...
        pv = survey_models.Video.objects.filter(id=self.pyano_video_id)
        if pv.count() == 0:
            logger.debug("Corresponding PYANO video is not found!")
            return
        p = pv[0]
...
def test_load():
    loader = Loader(location=os.path.abspath('./static/frames/fs-ifFsc8Bg/'),
